...
async function exportMarkedPDF() {
    ...
    await runExportTask(
        [exportBtn, exportPdfFromStatsBtn],
        exportMarkedPDFCore,
        '正在导出标注PDF…',
        'PDF 导出完成',
        '导出PDF失败，请检查网络后重试（需加载 pdf-lib）'
    );
}
    ...
    for (const doc of documents) {
        ...
        for (let i = 0; i < pageCount; i++) {
            ...
            for (const m of pageMarkers) {
                ...
                const drawSize = text.length > 5 ? fontSize * 0.68 : (text.length > 4 ? fontSize * 0.78 : fontSize);
                ...
                const baselineY = pdfY - VISUAL_CENTER_OFFSET * drawSize;
